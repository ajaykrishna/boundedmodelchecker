...
let set_fname config (s:string) =  
  if Sys.file_exists s then config := {!config with f_name=s}	else 					      raise (NotFound s) 
					   
let make_default_config () = {f_name="";debug=true;version="0.1";}
			       
let print_config cf =
  Printf.printf "inputfile=%s" cf.f_name
			       
let read_args () =
  let cf = ref (make_default_config()) in
  let speclist = 
    [
      ("--version",Arg.Unit (fun () -> fprintf std_formatter "HornAnalyser Version %s@." !cf.version ; raise(Version)),": print version and exit");
...
let _  =
  ...
  (*Abstract semantics*)
  Bmc.bmc_program stdout simple_java_prog;
