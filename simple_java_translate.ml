...
(* TODO LATER *)
(*     let atype = a.array_type in
     begin match atype with
     | Array_type(Integer(_)) -> 
	let asize = List.length (a.array_init) in
	if (asize == 0) then non_supported "not initialized array"
	else
	  non_supported "initialized array todo"
     (*	  Se_var (tr_variable_name env a);*)
     | _ -> non_supported "(nonint) array"
     end
 *)
  | Integer_constant i ->
      Se_const (Sc_int i)
